...
RSpec.describe OpeningHour, type: :model do
  ...
  context "Factory" do
    ...
    context 'with overlapping hours' do
      let!(:opening_hour) { create(:default_opening_hour, day: opening_hour_overlapping_opening_hours.day, shop_id: shop.id) }
        it 'exists one overlapping record in test database' do
          ...
          expect(OpeningHour.for_shop(shop).for_day(opening_hour.day).overlapping(opening_hour_overlapping_opening_hours.opens_at, opening_hour_overlapping_opening_hours.closes_at).count).to eq(1)
        end
        it 'is invalid' do
